...
from gym_connect_four.connect_four_render import render_board
...
class ConnectFourEnv(gym.Env):
	...
	def __init__(self, board_shape=(6, 7), window_width=512, window_height=512, win_condition=4):
	...
	def is_valid_action_for_state(self, state, action: int) -> bool:
	...
	def is_win_state(self) -> bool:
		...
		for i in range(self.board_shape[0]):
			for j in range(self.board_shape[1] - self.win_condition - 1):
				value = sum(self.__board[i][j:j + self.win_condition])
		...
		for i in range(self.board_shape[1]):
			for j in range(self.board_shape[0] - self.win_condition - 1):
				value = sum(reversed_board[i][j:j + self.win_condition])
				if abs(value) == self.win_condition:
					return True

		# Test diagonal
		for i in range(self.board_shape[0] - self.win_condition - 1):
			for j in range(self.board_shape[1] - self.win_condition - 1):
				value = 0
	...
	def available_moves_for_state(self, state) -> frozenset:
		return frozenset(i for i in range(len(state[0])) if self.is_valid_action_for_state(state, i))